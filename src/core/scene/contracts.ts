...
export type PickingHit =
  | { kind: "series-point"; seriesId: SeriesId; index: number }
  // A grab on a thin object (guide/segment) or on the BORDER of a filled
  // area — draggable to move.
  | { kind: "object"; objectId: ObjectId }
  // The interior/body of a filled area (rect/band) — selectable but
  // transparent to dragging, so a drag over it pans the plot.
  | { kind: "object-area"; objectId: ObjectId }
  | { kind: "object-handle"; objectId: ObjectId; handleId: number };
...
export function objectHandlePickingEntry(
  objectId: ObjectId,
  handle: ObjectHandle,
): PickingEntry {
  return {
    ...
    x: handle.x,
    y: handle.y,
    sizePx: handle.sizePx,
    offsetXPx: handle.offsetXPx,
    offsetYPx: handle.offsetYPx,
  };
}
